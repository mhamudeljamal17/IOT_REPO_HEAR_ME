...
int interprets_output(TfLiteTensor *outputTensor) {
    float threshold=0.5;
    int result = -1;               // -1 = uncertain
    float output = outputTensor->data.f[1];
    ...
    Serial.print("Sigmoid output f[1]: ");
    Serial.println(output, 9);

    
    Serial.print(" f[0]: ");
    Serial.println(outputTensor->data.f[0], 9);
    if (result != -1) {
        Serial.print("Predicted emotion: ");
        Serial.println(emotion_vec[result]);
    } else {
        Serial.println("Prediction uncertain, ignoring");
    }
