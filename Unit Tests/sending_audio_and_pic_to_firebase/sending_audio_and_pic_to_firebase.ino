...
#define SAMPLE_RATE     16000
#define RECORD_SECONDS  5
#define BUFLEN          (SAMPLE_RATE * RECORD_SECONDS)
#define I2S_DOUT        41
#define I2S_BCLK        42
#define I2S_LRCL        40
#define MIC_GAIN        4   // Digital gain
...
void recordAudio() {
  ...
  audio_size = BUFLEN * 2;  // 16-bit mono
  audio_buffer = (uint8_t *)heap_caps_malloc(audio_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!audio_buffer) {
    Serial.println("[MIC][ERROR] Audio buffer allocation failed");
    return;
  }
  ...
  while (offset < BUFLEN) {
    size_t to_read = min((size_t)512, BUFLEN - offset);
    i2s_read(i2s_num, temp, to_read * sizeof(int16_t), &bytes_read, portMAX_DELAY);

    // Apply digital gain
    for (int i = 0; i < bytes_read / 2; i++) {
      int32_t amplified = temp[i] * MIC_GAIN;
      if (amplified > 32767) amplified = 32767;
      if (amplified < -32768) amplified = -32768;
      temp[i] = amplified;
    }

    memcpy(audio_buffer + offset * 2, temp, bytes_read);
    offset += bytes_read / 2;
  }
